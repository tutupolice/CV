...
def load_lbl2id_map(lbl2id_map_path):
    with open(lbl2id_map_path, 'r', encoding='utf-8') as f:
        ...
        for line in reader:
            line = line.strip()
            if not line or '\t' not in line:  # 改为检查制表符
                continue
            label, id = line.split('\t')      # 使用制表符分割
            lbl2id_map[label] = int(id)
            id2lbl_map[int(id)] = label
...
class Recognition_Dataset(object):
    ...
    def __getitem__(self, index):
        ...
        #显示替换所有空白字符为普通空格
        lbl_str = lbl_str.replace('\u3000', ' ')  # 替换全角空格
        lbl_str = lbl_str.replace('\t', ' ')  # 替换制表符
        # ----------------
        # 图片预处理
        # ----------------
        # load image
...
if __name__ == "__main__":
    ...
    # TODO set parameters
    base_data_dir = r"D:\python3.12.3\CV-transformer\ICDAR_2015"  # 数据集根目录，请将数据下载到此位置
